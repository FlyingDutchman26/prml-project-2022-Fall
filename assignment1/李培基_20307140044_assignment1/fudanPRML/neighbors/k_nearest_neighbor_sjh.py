...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dists[i,j] = np.sqrt(np.power(sub,2).sum())
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            #######################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            sub =np.subtract(X[i] , self.X_train)
            temp = np.sum(np.power(sub,2), axis = 1)
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        X2 = np.power(X,2).sum(axis = 1).reshape(-1,1)
        Y2 = np.power(self.X_train , 2).sum(axis = 1).reshape(-1,1)
        XY = np.matmul(X,self.X_train.T)
        temp = X2 + Y2.T
        dists = np.sqrt(temp - 2*XY)
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            # TODO:
            #
            # 现在，你已经找到了k个最近邻的标签，接着需要在closest_y中找到最可能的标签。
            # 将此标签存储在y_pred [i]中。如果有两个标签可能性一样的话选择索引更小的那个。
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
